...
@export var inventory : Dictionary = {
	# Ordem do projeto: C19, SMG45, AK, LMG63, Sawnoff e faca.
	"weapons" = [[1,-1], [2,-1], [3,-1], [4,-1], [5,-1], [0,-1]],
	"ammo" = {
		"none" = 0,
		"9mm" = 240,
		"rifle" = 600,
...
@export_range(0.0, 20.0, 0.5) var ads_fov_reduction : float = 8.0
...
func _enter_tree() -> void:
	...
	MeleeRayCast.target_position = Vector3(0, 0, -3.0)
	MeleeRayCast.collision_mask = collision_mask
	MeleeRayCast.collide_with_areas = true
	...
	FireRayCast.collide_with_areas = true
...
func add_decal(pos:Vector3, normal:Vector3, decal:PackedScene, father:Node3D) -> void:
	...
	var tangent := surface_normal.cross(Vector3.UP)
	if tangent.length_squared() < 0.001:
		tangent = surface_normal.cross(Vector3.RIGHT)
	tangent = tangent.normalized()
	var bitangent := tangent.cross(surface_normal).normalized()
	# Decal projeta do +Y local para o -Y local. O eixo Y precisa apontar
	# para fora da superfície, na mesma direção do normal da colisão.
	bullethole.global_basis = Basis(tangent, surface_normal, bitangent).orthonormalized()
	bullethole.global_position = pos + surface_normal * 0.002
	bullethole.rotate_object_local(Vector3.UP, randf_range(0.0, TAU))
	var lifetime := get_tree().create_timer(45.0)
	lifetime.timeout.connect(bullethole.queue_free)

func melee_attack(raycast:RayCast3D, damage:float) -> void:
	raycast.force_raycast_update()
	var collider := raycast.get_collider() as Node3D
	if collider != null:
		_apply_melee_hit(collider, damage, raycast.get_collision_point(), raycast.get_collision_normal())


func melee_camera_attack(damage: float, distance: float) -> void:
	if not camera or not camera.get_world_3d():
		melee_attack(MeleeRayCast, damage)
		return
	...
	while ancestor:
		if ancestor is CollisionObject3D:
			excluded.append((ancestor as CollisionObject3D).get_rid())
	...
	if collider:
		_apply_melee_hit(collider, damage, hit["position"] as Vector3, hit["normal"] as Vector3)


func _apply_melee_hit(collider: Node3D, damage: float, point: Vector3, normal: Vector3) -> void:
	give_damage.emit(collider, damage * damage_multiplier, point)
	if collider is StaticBody3D or collider is RigidBody3D:
		add_decal(point, normal, scratchScene, collider)
...
func fire_bullet(look_at_dir:Vector3, damage:float, applies_damage:bool = true) -> void:
	if muzzlePoint:
		...
		bullet_instance.damage = damage if applies_damage else 0.0
		bullet_instance.distance = weapon.get_meta("range")
		bullet_instance.collision_mask = collision_mask
		bullet_instance.shooter = get_parent().get_parent() as CollisionObject3D
...
func fire_hitscan(origin:Vector3, direction:Vector3, distance:float, damage:float) -> Vector3:
	...
	if hit.is_empty():
		return query.to
	var collider := hit.get("collider") as Node3D
	var point := hit.get("position", query.to) as Vector3
	var normal := hit.get("normal", Vector3.UP) as Vector3
	if collider:
		give_damage.emit(collider, maxf(damage * damage_multiplier, 0.0), point)
		if collider is StaticBody3D:
			add_decal(point, normal, bulletholeScene, collider)
	return point
...
func fire_spread(raycast:RayCast3D, bullets_count:int, spread_deg:float, damage:float) -> void:
	var aim_transform := camera.global_transform if camera else raycast.global_transform
	var ray_origin := aim_transform.origin
	var ray_forward := -aim_transform.basis.z.normalized()
	var distance := float(weapon.get_meta("range", 100.0))
	var dir : Vector3

	for i in bullets_count:
		dir = random_direction_in_cone(ray_forward, spread_deg)
		fire_hitscan(ray_origin, dir, distance, damage)

func fire_single(raycast:RayCast3D, spread_deg:float, multiplier:float, damage:float) -> void:
	var aim_transform := camera.global_transform if camera else raycast.global_transform
	var ray_forward: Vector3 = -aim_transform.basis.z.normalized()
	var ray_up: Vector3      = aim_transform.basis.y.normalized()
	var ray_right: Vector3   = -aim_transform.basis.x.normalized()
	var distance := float(weapon.get_meta("range", 100.0))
	# Random offsets
	...
	fire_hitscan(aim_transform.origin, dir, distance, damage)
...
func take_weapon(inventory_index:int) -> void:
	...
	start_rotation = weapon.get_meta("hip_rotation", weapon.rotation)
	ads_rotation = weapon.get_meta("ads_rotation", start_rotation)
	weapon.rotation = start_rotation
	...
	FireRayCast.target_position = Vector3(0, 0, -float(weapon.get_meta("range")))
...
func _process(delta) -> void:
	if weapon != null:
		...
		if ads and (weapon.position != ads_pos or weapon.rotation != ads_rotation):
			...
			weapon.rotation = weapon.rotation.lerp(ads_rotation, weapon.get_meta("delta",1.5)*delta_multiplier*delta)
		if !ads and (weapon.position != start_pos or weapon.rotation != start_rotation):
			weapon.position = weapon.position.move_toward(start_pos, weapon.get_meta("delta",1.5)*delta_multiplier*delta)
			weapon.rotation = weapon.rotation.lerp(start_rotation, weapon.get_meta("delta",1.5)*delta_multiplier*delta)
		if ads_dynamic_fov and camera:
			var target_fov := start_fov - ads_fov_reduction if ads else start_fov
...
func _physics_process(delta) -> void:
	...
	# Shake
		if shake and camera and shake_current > 0:
			...
			camera_shake()
		elif shake and camera:
			camera.rotation.z = move_toward(camera.rotation.z, 0.0, delta * 4.0)
			camera.h_offset = move_toward(camera.h_offset, 0.0, delta * 0.1)
			camera.v_offset = move_toward(camera.v_offset, 0.0, delta * 0.1)
...
func _on_animation_tree_animation_started(anim_name: StringName) -> void:
	...
	if anim_name == "fire":
		...
		if weapon.get_meta("melee", false):
			melee_camera_attack(float(weapon.get_meta("damage")), float(weapon.get_meta("range", 3.0)))
		else:
	...
	if anim_name == "melee":
		meleeing.emit()
		melee_camera_attack(melee_damage, 3.0)
	if anim_name == "take":
		taking_weapon.emit()
	if anim_name == "reload" or anim_name == "reload_full":
		reloading.emit()
